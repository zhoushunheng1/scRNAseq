...
features <- rownames(Stroke)[rownames(Stroke) %in% retained_genes]
Stroke <- subset(Stroke, features = features)

Idents(Stroke)<-Stroke$stim
DEG_method<-"wilcox"
Stroke <- FindVariableFeatures(Stroke, selection.method = "vst", nfeatures = 2000)
ALL_DEGs <- FindMarkers(Stroke, ident.1 = "Stroke", ident.2 = NULL,test.use = DEG_method,logfc.threshold =0,    min.pct = 0)
sig_DEGs <- FindMarkers(Stroke, ident.1 = "Stroke", ident.2 = NULL,test.use = DEG_method,logfc.threshold = 0.25,min.pct = 0.1)
